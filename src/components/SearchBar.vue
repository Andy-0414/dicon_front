<template>
    <div>
        <v-navigation-drawer clipped fixed v-model="drawer" app width="350">
            <v-list two-line class="mx-2">
                <v-subheader class="title mt-3 grey--text text--darken-5">조건</v-subheader>
                <v-list-tile>
                    <v-autocomplete :items="degreeItem" label="참여대상"></v-autocomplete>
                </v-list-tile>
                <v-list-tile>
                    <v-select :items="ableItem" label="신청가능"></v-select>
                </v-list-tile>
                <v-subheader class="title mt-5 grey--text text--darken-5">기한</v-subheader>
                <v-list-tile>
                    <v-menu :close-on-content-click="false" :nudge-right="40" lazy transition="scale-transition"
                        offset-y full-width min-width="290px">
                        <v-text-field slot="activator" v-model="startDate" label="시작 날짜" prepend-icon="event" readonly></v-text-field>
                        <v-date-picker ref="picker" v-model="startDate" @change="save" :max="endDate"></v-date-picker>
                    </v-menu>
                </v-list-tile>
                <v-list-tile>
                    <v-menu :close-on-content-click="false" :nudge-right="40" lazy transition="scale-transition"
                        ...
                        <v-date-picker ref="picker" v-model="endDate" @change="save"></v-date-picker>
                    </v-menu>
                </v-list-tile>
